...
class provSWregister extends openSRS_base {
	...
	private function _createUserData($contactSet = null){
		if (isset($contactSet, $this->_dataObject->{$contactSet})) {
			$contactObj = $this->_dataObject->{$contactSet};
		} else {
			$contactObj = $this->_dataObject->personal;
		}
		$userArray = array(
			"first_name" => $contactObj->first_name,
			"last_name" => $contactObj->last_name,
			"org_name" => $contactObj->org_name,
			"address1" => $contactObj->address1,
			"address2" => $contactObj->address2,
			"address3" => $contactObj->address3,
			"city" => $contactObj->city,
			"state" => $contactObj->state,
			"postal_code" => $contactObj->postal_code,
			"country" => $contactObj->country,
			"phone" => $contactObj->phone,
			"fax" => $contactObj->fax,
			"email" => $contactObj->email,
			"url" => $contactObj->url,
			"lang_pref" => $contactObj->lang_pref
		);
		return $userArray;
	}
}
